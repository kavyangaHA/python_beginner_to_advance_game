...
class Fire:
    def __init__(self,x,y):
        self.image = load_image("tochLit2.png")
...
#variables for indicating levels
...
def reset_game(level):
    global player, enemies,fires, coin, score,player_lives,current_level
    player = Player()
    #enemy = Enemy()
    coin = Coin()
    score = 0
...
##            reset_game()  # Reset the game for a new start
        ...
        #adding Fire rain for level - Hard

        if current_level == "Hard":
            if random.randint(1,60) == 2: #1 in 60 chance each frame
        ...
        #Draw button to restart/quit
        restart_text = info_font.render("Play Again",True,black)
        restart_rect = restart_text.get_rect(center=beginner_button.rect.center)
        screen.blit(restart_text, restart_rect)

        quit_text = info_font.render("Main Menu",True,black)
        quit_rect = quit_text.get_rect(center=intermediate_button.rect.center)
